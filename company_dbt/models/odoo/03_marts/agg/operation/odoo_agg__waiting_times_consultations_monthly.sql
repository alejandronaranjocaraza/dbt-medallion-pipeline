...
group by date_month, waiting_category
),
base as (
select
  dd.date_month,
  wtc.* as waiting_category
from (select distinct date_month from dim_date) dd
...
from base b
left join monthly_waiting_times mwt
using (date_month, waiting_category))
select
  date_month,
  waiting_category,
  qty_appointments,
